...
public class AvroIngest837Serializer extends AbstractAvroEventSerializer<AvroIngest837Serializer.Container> {
  ...
  HTable table = null;
  private AvroIngest837Serializer(OutputStream out) {
    this.out = out;
  }
  ...
  public void write(Event event) throws IOException {
	  ...
	  GenericRecordBuilder recordBuilder = new GenericRecordBuilder(getSchema());
		...
		System.out.println("segment separator is " + segSeparator);
		...
		for (int i=0;i<strHeader.length;i++){
			...
			//System.out.println("Key -" + strHeadSplit[0] + "_" + strHeadSplit[1] + " : Value - " + strHeader[i]);
			for (int j=1;j<strHeadSplit.length;j++){
				//System.out.println("Key - " + strHeadSplit[0] + "_" + strHeadSplit[1] + "_" + j + " : value - " + strHeadSplit[j]);
...
//		for (String strTran : strST){
		...
		/*	if (strTransaction.indexOf(segSeparator+"LX*") == -1){
			...
			strTranDetailsRaw = strTransaction.substring(0, strTransaction.indexOf(segSeparator+"CLM*")+1);	
			//}
			...
			/*int intEndOfST = strTran.indexOf(segSeparator+"PLB*"); 
			if (intEndOfST < 0){*/
			int intEndOfST = strTran.indexOf(segSeparator+"SE*");
			//}
			...
			String[] strCLM = strTransaction.split(segSeparator+"CLM\\*");
			
			for (int i=1;i<strCLM.length;i++){
				x12Body.clear();
				x12Body.put("RAWCLM", "CLM*"+strCLM[i]);
				String[] strCLEach = ("CLM*"+strCLM[i]).split(segSeparator);
				Iterator<String> keySetIterator = x12Body.keySet().iterator();
	  			for (int k=0;k<strCLEach.length;k++){
	      			String[] strCL = strCLEach[k].split("\\*");
	      			if (strCL[0].trim().equalsIgnoreCase("CLM")){
...
@Override
  ...
  public static class Builder implements EventSerializer.Builder {
    ...
    public EventSerializer build(Context context, OutputStream out) {
      AvroIngest837Serializer writer = new AvroIngest837Serializer(out);
